...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        jQuery(".btn-popup").click(function (e) {
            if ($(this).hasClass('third-btn-popup')) {
                if ($(this).hasClass('active')) {
                    $(this).removeClass('active')
                    $('.third-popup-wrap').removeClass('is-visible');
                } else {
                    $(this).addClass('active')
                    $('.third-popup-wrap').addClass('is-visible');
                }
            }
        });
        jQuery(".popup-close").click(function (e) {
            $('.btn-popup').removeClass('active')
            $(this).parents('.landing-popup-wrap').removeClass('is-visible');
        });
        ...
        $('.content--grid .lists, .page-short-story .image_block_lists').each(function(){
            var wrapIT = $(this).children('div.item');
        ...
        if (winWidth <= 768) {
            $(document).ready(function () {
                ...
                var viewBox1 = check('.new-popup:not(.static-popup) .viewBox');
                console.log(viewBox1);
                if (viewBox1 > 0) {
                    var viewportHeight = $('.new-popup').height(),
                            viewportMidHeight = parseInt(viewportHeight / 2);
                    concat = 'div.viewBox:in-viewport' + '(' + viewportMidHeight + ')';
                    concatvid = concat + ' video';
                    // Play first video in viewport -----------------------------------------
                    if ($(concatvid).length >= 1) {
                        $(concat).find('.loader').css('opacity', 1).css('visibility', 'visible');
                        $('.new-popup').find(concatvid).get(0).play();
                        $('.new-popup').find(concatvid).css('opacity', 1);
                    }
                    // Handle video in viewport on scroll -----------------------------------
                    $('.new-popup').scroll(function () {
                        $('div.viewBox').find('.loader').css('opacity', 0).css('visibility', 'hidden');
                        $('div.viewBox').removeClass('inView').find('video').css('opacity', 0).get(0).pause();
                        $(concat).find('.loader').css('opacity', 1).css('visibility', 'visible');
                        $(concat).addClass('inView').find('video').css('opacity', 1);
                ...
                $('.horizon-prev').on('click', function () {
                    $('.plan-client-table').animate({
                      scrollLeft: "-=775px"
                    }, "slow");
                    $(this).fadeOut();
                    $('.horizon-next').fadeIn();
                    $('.plan-client-table').removeClass('scroll-active');
                  });
                   ...
                   $('.horizon-next').on('click', function () {
                    ...
                    $(this).fadeOut();
                    $('.horizon-prev').fadeIn();
                    $('.plan-client-table').addClass('scroll-active');
                  });
                  $('.plan-client-table').on('click', function () {
                      if ($(this).hasClass('scroll-active')) {
                          $(this).removeClass('scroll-active');
                        $('.plan-client-table').animate({
                            scrollLeft: "-=775px"
                           }, "slow");
                        $('.horizon-prev').fadeOut();
                        $('.horizon-next').fadeIn();
                      } else {
                        ...
                        $('.horizon-next').fadeOut();
                        $('.horizon-prev').fadeIn();
                      } 
                  });
                  var table_div = $(window).width()- 50;
                  var table_width = 265 + table_div;
                  var table_3td = table_div/2.1;
                  var table_4td = table_div/1.9;
                  $('.plan-client-table table').width(table_width);
                  $('.plan-client-table table tr td:nth-child(3)').width(table_3td);
                  $('.plan-client-table table tr td:nth-child(5)').width(table_4td);
        ...
        //Read querystring and append parameters in iframe src
        var qs = (function (a) {
            if (a == "")
                return {};
            var b = {};
        ...
        if (qs['utm_source']) {
            cookieCreate('utm_source', qs['utm_source']);
        }
        if (qs['utm_medium']) {
            cookieCreate('utm_medium', qs['utm_medium']);
        }
        if (qs['utm_campaign']) {
            cookieCreate('utm_campaign', qs['utm_campaign']);
        }
        if (qs['utm_term']) {
            cookieCreate('utm_term', qs['utm_term']);
        }
        if (qs['gclid']) {
            cookieCreate('gclid', qs['gclid']);
        }
        if (qs['msclkid']) {
            cookieCreate('msclkid', qs['msclkid']);
        }
    ...
    $(window).load(function () {
        ...
        // 	});
        // }, 3000);
        //console.log($('#book-workout-form').length);
        if ($('#book-workout-form iframe').length > 0) {
            var utm_source = cookieRead('utm_source');
            var utm_medium = cookieRead('utm_medium');
            var utm_campaign = cookieRead('utm_campaign');
            var utm_term = cookieRead('utm_term');
            var gclid = cookieRead('gclid');
            var msclkid = cookieRead('msclkid');

            var q_string = '';
            if (utm_source) {
                q_string += 'utm_source=' + utm_source + '&';
            }
            if (utm_medium) {
                q_string += 'utm_medium=' + utm_medium + '&';
            }
            if (utm_campaign) {
                q_string += 'utm_campaign=' + utm_campaign + '&';
            }
            if (utm_term) {
                q_string += 'utm_term=' + utm_term + '&';
            }
            if (gclid) {
                q_string += 'gclid=' + gclid;
            } else if (msclkid) {
                q_string += 'msclkid=' + msclkid;
            }

            //console.log(q_string);
            if (q_string != '') {
                $('#book-workout-form iframe').attr('src', $('#book-workout-form iframe').attr('src') + '?' + q_string);
            }
        }
    });
    // Global Window Load --------------------------------
    ...
    jQuery(".interest-from .wpcf7-submit").on('click', function () {
        ...
        var your_department = jQuery('.your-department input').val();
        ...
        if( your_department == '' ) {
			jQuery('.your-department').append('<span role="alert" class="wpcf7-not-valid-tip">The field is required.</span>');
		};
