...
jQuery(window).ready(function(){
	...
	if(modal == 'special'){
		...
		jQuery('body').addClass('open-modal open-landing open-bootcamp');
	}
	if(modal == 'gym'){
		<?php $page4 = get_page_by_title( 'No Bootcamp' ); ?>
		jQuery('body').addClass('open-modal open-landing open-nobootcamp');
	}
	jQuery('.close-home-popup').on('click', function(){
		jQuery('body').removeClass('open-modal open-gym-near-me open-booking open-landing open-nobootcamp open-bootcamp');
		jQuery('.hide-div').hide();
	});
	//form parameter
...
<div class="landing-popup home-popup new-popup">
	<div class="popup-content home-popup-content">
				...
				<div class="flex-wrap">
							...
							<div class="flashsale-wrap">
								<div class="flashsale-text">
									<div class="arrow-list">
										<div class="show-bootcamp">
											<?php echo get_field('sub_description', $page3->ID); ?>
										</div>
										<div class="show-nobootcamp">
											<?php echo get_field('sub_description', $page4->ID); ?>
										</div>
									</div>
								</div>
							</div>
					...
					<div class="col col-form">
						<?php 
							...
							$form_title_nobootcamp = get_field('form_title', $page4->ID);
						?>
						<div class="bg-white">
						<h4>STEP 1 OF 2</h4>
						<h3><div class="show-bootcamp"><?php echo $form_title; ?></div><div class="show-nobootcamp"><?php echo $form_title_nobootcamp; ?></div></h3>
		...
		<section class="featured content--grid animate section-list bg-grey">
			...
			<div class="show-nobootcamp">
				<div class="lists">
					<?php while( have_rows('video_list', $page4->ID) ): the_row(); 

					// vars
					$thumbnail = get_sub_field('image_list', $page4->ID);
					$title = get_sub_field('title_list', $page4->ID);
					$full_video = get_sub_field('full_video', $page4->ID);
					$video = get_sub_field('short_video', $page4->ID);
					if($full_video) {
						$videoclass = " play-video-popup";
					}else {
						$videoclass = "";
			...
			<div class="popup-btn">
				...
				<div class="show-nobootcamp">
				<a class="btn btn--primary btn--large scroll-top scroll-form" href="<?php echo get_field('big_button_url', $page4->ID); ?>"><?php echo get_field('big_button_text', $page4->ID); ?>
					<svg width="12px" height="15px" viewBox="0 0 12 15" version="1.1" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns="http://www.w3.org/2000/svg">
						<path d="M6.92779e-13 2.67133e-12L7.5591 5.24357L15 2.67133e-12L15 6.56304L7.5 12L8.52651e-13 6.56304L6.92779e-13 2.67133e-12Z" transform="matrix(-4.371139E-08 -1 1 -4.371139E-08 0 15)" fill="#1A1B1B" stroke="none"></path>
					</svg>
				</a>
...
<style>
	.hide-div, .show-bootcamp, .show-nobootcamp {
		display:none;
	}
	.open-modal {
		overflow:hidden;
	...
	.open-bootcamp .show-bootcamp, .open-nobootcamp .show-nobootcamp {
		display:block;
	}
	.flex-wrap {
		display: flex;
